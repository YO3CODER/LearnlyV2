...
function buildDeck(): MemoryCard[] {
  ...
  return shuffle(pairs).map((p, i) => ({ uid: i, ...p, flipped: false, matched: false }));
}
...
function MemoryGame({ onClose }: { onClose: () => void }) {
  ...
  const flip = useCallback((uid: number) => {
    ...
    if (newSelected.length === 2) {
      ...
      if (a.id === b.id) {
        setTimeout(() => {
          setCards(c => c.map(card => card.id === a.id ? { ...card, matched: true } : card));
          setSelected([]);
          setLocked(false);
        }, 400);
      } else {
        setTimeout(() => {
          setCards(c => c.map(card => newSelected.includes(card.uid) ? { ...card, flipped: false } : card));
          setSelected([]);
          setLocked(false);
        }, 900);
      }
...
// ─── GameCard — preview = gradient + titre centré ─────────────────────────────
function GameCard({
  title, badge, badgeColor, borderColor, description, gradient, onPlay,
}: {
  title: string;
  badge: string;
  badgeColor: string;
  borderColor: string;
  description: string;
  gradient: string;   // classe Tailwind du gradient ex: "from-violet-500 to-indigo-600"
  onPlay: () => void;
}) {
  return (
    <div className={`card-in bg-card rounded-2xl border border-border shadow-sm hover:shadow-lg transition-all duration-200 overflow-hidden border-l-4 ${borderColor} flex flex-col`}>
      ...
      {/* Preview : gradient + titre */}
      <div
        className={`relative overflow-hidden bg-gradient-to-br ${gradient} flex flex-col items-center justify-center gap-2`}
        style={{ height: 160 }}
      >
        {/* Cercles décoratifs */}
        ...
        <p className="relative z-10 text-white font-bold text-lg text-center px-3 leading-tight drop-shadow-md" style={fredoka}>
          {title}
        </p>
        <span className={`relative z-10 inline-block text-xs font-semibold px-2 py-0.5 rounded-full ${badgeColor}`} style={fredoka}>
          {badge}
        </span>
        ...
        <div
          onClick={onPlay}
          className="absolute inset-0 bg-black/0 hover:bg-black/30 transition-all duration-200 flex items-center justify-center group cursor-pointer"
        >
      ...
      <div className="px-4 py-3 flex flex-col gap-2 flex-1">
        ...
        <button onClick={onPlay} style={fredoka}
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm bg-violet-500 text-white border-b-4 border-violet-600 hover:bg-violet-500/90 active:border-b-0 transition-all mt-auto">
